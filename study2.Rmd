...
column_names <- names(read.csv("context effect in news choice S2_April 4, 2025_00.47.csv", nrows = 1))

df <- read.csv("context effect in news choice S2_April 4, 2025_00.47.csv", sep=",", skip = 80, header = FALSE)
...
plot_data <- filtered_df %>%
  ...
  group_by(Headline, Treatment, topic) %>%
  summarise(
    mean_rating = mean(as.numeric(rating), na.rm = TRUE),
    se_rating = sd(as.numeric(rating), na.rm = TRUE) / sqrt(n())
  )
...
library(lmerTest)
late_df <- filtered_df[filtered_df$presentation_order %in% c(3, 4), ]

# Fit model on subset
model1 <- lmer(rating ~ scale(prior)*headline_stat + (1|ResponseId), data = late_df)

# Add residuals to that subset only
late_df$residual_rating <- resid(model1)

model2 <- lmer(residual_rating ~ Treatment*headline_stat + (1|ResponseId), data = late_df)
summary(model2)
